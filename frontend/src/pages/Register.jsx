import { Home, LockKeyhole, Mail, PlugZap, UserRound, Wifi } from "lucide-react";
...
export default function Register({ onAuth, onSwitch }) {
  ...
  return (
    <main className="min-h-screen bg-[#f5f7f2] px-4 py-6 text-slate-950">
      <section className="mx-auto grid min-h-[calc(100vh-3rem)] w-full max-w-5xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm lg:grid-cols-[420px_1fr]">
        <div className="flex items-center justify-center p-5 sm:p-8">
          <div className="w-full max-w-sm">
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm text-slate-500">Start with your account, then add rooms and devices.</p>
            </div>
        ...
        <div className="hidden bg-slate-950 p-8 text-white lg:flex lg:flex-col lg:justify-between">
          <div>
            ...
            <h2 className="mt-6 text-4xl font-bold">Set up your smart home</h2>
            <p className="mt-4 max-w-sm text-base leading-7 text-slate-300">
              Add every room, attach devices, and switch them on or off from a responsive dashboard.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-md bg-white/10 p-4">
              <PlugZap className="h-6 w-6 text-emerald-300" />
              <p className="mt-3 text-2xl font-bold">Live</p>
              <p className="text-sm text-slate-300">Device states</p>
            </div>
            <div className="rounded-md bg-white/10 p-4">
              <Wifi className="h-6 w-6 text-emerald-300" />
              <p className="mt-3 text-2xl font-bold">All</p>
              <p className="text-sm text-slate-300">Rooms synced</p>
            </div>
          </div>
        </div>
      </section>
